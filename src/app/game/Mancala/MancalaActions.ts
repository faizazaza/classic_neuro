...
export const mancalaSocketTexts = {
    start: (startPit: number, endPit: number, storeNum: number, boardState: string) => 
        `To play Mancala, you must take turns selecting one of your pits and distributing its seeds one by one into the following pits around the board.
            ...
            Your pits are ${startPit} to ${endPit} inclusive.
    ...
    turn: (startPit: number, endPit: number, storeNum: number, storedSeeds: number, boardState: string) => 
        `It is your turn, use the pick_pit action to move the seeds from any of the pits ${startPit} to ${endPit} inclusive. 
    ...
    resultPlayer: (chosenPit: number, storedSeeds: number, boardState: string) => 
        `The seeds in your selected pit ${chosenPit} were moved successfully. 
            Your store contains ${storedSeeds} seeds. New board state is: ${boardState}`,
    resultOpponent: (chosenPit: number, storedSeeds: number, boardState: string) => 
